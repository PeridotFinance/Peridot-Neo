...
export default function IFO() {
  const chain = getChains()[0].nameId;
  const collection = getCollection(chain, "MAYC");
  ...
  function refreshTokenBalance() {
    if (isDefined(balances)) {
      let tokenBalance = getBalance(payToken);
  ...
  function refreshMintPrice() {
    if (mintAmount > 0) {
      // Parse the price as a float and multiply by mintAmount
      const newPrice = parseFloat(collection.price) * mintAmount;
      // Ensure the price is formatted to two decimal places
      setMintPrice(newPrice.toFixed(2));
    } else {
      setMintPrice("0");
    }
  }
