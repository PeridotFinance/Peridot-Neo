...
async function main() {
  ...
  console.log("Calling projectToMiniNFT...");

  // Call the function
  const tx = await peridotFactory.projectToMiniNFT(projectAddress);

  const mininftAddress = await peridotFactory.projectToMiniNFT(projectAddress);

  console.log("MiniNFT Address for this project is:", mininftAddress);
}
