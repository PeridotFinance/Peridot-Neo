...
interface IMasterchefV2 {
    ...
    function peridotPerBlock() external view returns (uint256);
}

// File contracts/PeridotBoost.sol

pragma solidity ^0.8.4;

/**
 * @dev Stake xPeridot to earn more Peridot
 */
...
contract PeridotBoost is ERC20, Ownable, ReentrancyGuard {
    ...
    constructor(
        IERC20 _peridotToken
    ) Ownable(msg.sender) ERC20("PeridotBar", "xPeridot") {
        peridotToken = _peridotToken;
    }
    ...
    function stake(
        ...
        peridotToken.transferFrom(msg.sender, address(this), _amount);

        uint256 multiplier = getMultiplier(_lockingPeriod);
        require(multiplier > 0, "Invalid multiplier");

        uint256 xPeridotAmount = (_amount * multiplier) / 100;

        Staker storage staker = stakers[msg.sender];
        staker.amountStaked += _amount;
        staker.lockingPeriod = _lockingPeriod;
        staker.rewardMultiplier = multiplier;
        staker.stakingTime = block.timestamp;

        _mint(msg.sender, xPeridotAmount);

        emit Staked(msg.sender, _amount, _lockingPeriod);
    }
    ...
    function unstake(uint256 _amount) external nonReentrant {
        ...
        require(
            block.timestamp >=
                staker.stakingTime + staker.lockingPeriod * 1 days,
            "Locking period not finished"
        );
        require(
            _amount <= staker.amountStaked,
            "Cannot unstake more than staked"
        );

        _burn(msg.sender, _amount * staker.rewardMultiplier);

        peridotToken.transfer(msg.sender, _amount);

        staker.amountStaked -= _amount;
        if (staker.amountStaked == 0) {
            staker.lockingPeriod = 0;
    ...
    function getMultiplier(
        ...
        if (_lockingPeriod == 30) return 100;
        if (_lockingPeriod == 90) return 120;
        if (_lockingPeriod == 180) return 150;
        return 0; // This line is just a fallback, should not be reached due to validation in `stake`
    }
}
